...
BayesianNormal <- function(times, X, model = c("OU", "CIR"), prior, start, random, nMCMC = 1000, propSd = 0.2) {
    ...
    if (model == "OU") {
        ...
        postSigma <- function(phi, sigmaOld = 1, propSdSigma2 = NULL) {
            ...
            help <- numeric(M)
            for (i in 1:M) {
                help[i] <- sum(phi[i, 2]/(1 - exp(-2 * phi[i, 2] * delta)) * (X[i, -1] - phi[i, 1]/phi[i, 2] - (X[i, -N] - phi[i, 1]/phi[i, 2]) * exp(-phi[i, 2] * delta))^2)
            }
        ...
        likeli <- dcCIR2
        postSigma <- function(phi, sigmaOld = 1, propSdSigma2 = NULL) {
            # alphaPost <- prior$alpha.sigma + M*(N-1)/2 help <- numeric(M) for (i in 1:M) { help[i] <- sum((X[i,-1] - X[i,-N] - b(phi[i, ], times[-N], X[i,-N]) * delta)^2/
            # (X[i,-N] * delta)) } betaPost <- prior$beta.sigma + sum(help)/2 return(1/rgamma(1, alphaPost, betaPost))
    ...
    if (length(random) == 2) {
        ...
        for (count in 1:nMCMC) {
            ...
            if (count%%50 == 0){
              propSdPhi <- c(
                ad.propSd_random(alpha_out[(count-50+1):count, ], propSdPhi[1], count),
                ad.propSd_random(beta_out[(count-50+1):count, ], propSdPhi[2], count)
              )
            }
        ...
        if (random == 1) {
            for (count in 1:nMCMC) {
                ...
                if (count%%50 == 0){
                  propSdPhi <- c(
                    ad.propSd_random(alpha_out[(count-50+1):count, ], propSdPhi[1], count),
                    ad.propSd(beta_out[(count-50+1):count], propSdPhi[2], count)
                  )
                }
        ...
        if (random == 2) {
            ...
            for (count in 1:nMCMC) {
                ...
                if (count%%50 == 0){
                  propSdPhi <- c(
                    ad.propSd(alpha_out[(count-50+1):count], propSdPhi[1], count),
                    ad.propSd_random(beta_out[(count-50+1):count, ], propSdPhi[2], count)
                  )
                }
...
diagnostic <- function(results, random) {
    if (length(random) == 2) {
        # he <- matrix(0,5,4) he[1, ] <- raftery.diag(as.mcmc(results$mu[,1]))$resmatrix he[2, ] <- raftery.diag(as.mcmc(results$mu[,2]))$resmatrix he[3, ] <-
        # raftery.diag(as.mcmc(results$omega[,1]))$resmatrix he[4, ] <- raftery.diag(as.mcmc(results$omega[,2]))$resmatrix he[5, ] <-
        ...
        if (random == 1) {
            # he <- matrix(0,4,4) he[1, ] <- raftery.diag(as.mcmc(results$mu))$resmatrix he[2, ] <- raftery.diag(as.mcmc(results$beta))$resmatrix he[3, ] <-
            # raftery.diag(as.mcmc(results$omega))$resmatrix he[4, ] <- raftery.diag(as.mcmc(results$sigma2))$resmatrix burnIn <- max(he[,1]) thinning <-
            ...
            # he <- matrix(0,4,4) he[1, ] <- raftery.diag(as.mcmc(results$mu))$resmatrix he[2, ] <- raftery.diag(as.mcmc(results$alpha))$resmatrix he[3, ] <-
            # raftery.diag(as.mcmc(results$omega))$resmatrix he[4, ] <- raftery.diag(as.mcmc(results$sigma2))$resmatrix burnIn <- max(he[,1]) thinning <-
            # floor(length(results$sigma2)/max(he[,3]))
            he <- matrix(0, 4, 2)
            he[1, ] <- diagnostic.own(results$mu)
